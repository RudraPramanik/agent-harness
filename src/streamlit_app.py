...
async def main() -> None:
    ...
    with st.sidebar:
        st.markdown(
            f'<p class="studio-title">{APP_ICON} {APP_TITLE}</p>'
            f'<p class="studio-tagline">{APP_TAGLINE}</p>'
            f'<p class="studio-tagline"><em>LangGraph · FastAPI · Streamlit</em></p>',
        ...
        def architecture_dialog() -> None:
            arch_url = f"{GITHUB_RAW_BASE}/media/agent_architecture.png?raw=true"
            st.image(arch_url)
            st.markdown(f"[View full size on GitHub]({GITHUB_RAW_BASE}/media/agent_architecture.png)")
            st.caption(
                f"{APP_TITLE} — FastAPI agent service with Streamlit chat UI. "
                "Deploy the API and UI separately for production."
            )
    ...
    if len(messages) == 0:
        WELCOME = WELCOME_MESSAGES.get(agent_client.agent or "", DEFAULT_WELCOME)
