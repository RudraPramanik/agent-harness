...
The CI workflow SHALL run without production LLM or database secrets; only optional GitHub secrets listed below may be configured.

| Secret | Required | Purpose |
|--------|----------|---------|
| `CODECOV_TOKEN` | No | Upload coverage to Codecov |

No LLM API keys are required in GitHub Actions for CI (tests use `USE_FAKE_MODEL=true`).
...
The system SHALL support optional auto-deploy of the API to Render after CI passes on `main`.
...
- **THEN** operator deploys via Render dashboard connected to GitHub (auto-deploy on push) or manual deploy
...
- **THEN** Streamlit Cloud rebuilds the app (may take a few minutes)
